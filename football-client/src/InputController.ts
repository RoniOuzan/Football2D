...
export default class InputController {
  ...
  constructor(client: Client) {
    ...
    window.addEventListener("keydown", e => this.kbKeys.add(e.key.toLowerCase()));
    window.addEventListener("keyup", e => this.kbKeys.delete(e.key.toLowerCase()));
  ...
  private sendAllInputs() {
    ...
    this.client.sendDevices(devices);
  }

  private mapController(gp: Gamepad): DeviceInput {
    const inputs = new Set<string>();
    const axes: Axes = { leftX:0, leftY:0, rightX:0, rightY:0, LT:0, RT:0 };

    const name = gp.id.toLowerCase();
    const isPS = name.includes("dualshock") || name.includes("wireless") || name.includes("ps");

    const MAP_X = ["A","B","X","Y","LB","RB","LT","RT","BACK","START","LS","RS","UP","DOWN","LEFT","RIGHT"];
    ...
    gp.buttons.forEach((b,i)=>{
      if(i===6) axes.LT = b.value;
        ...
        const name = isPS ? MAP_PS[i] : MAP_X[i];
